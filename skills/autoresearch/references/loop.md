...
During the loop the skeleton changes only in two bounded ways — a frozen counterexample may add one example-TODO (small, no re-review), and **plateau-break** rewrites it from scratch (re-review). There is **no per-cycle skeleton review.**
