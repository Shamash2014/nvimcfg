...
Phase 4 is **autonomous within each handoff's `max_cycles`** (no per-cycle permission) with exactly **one** mid-execution stop: the atomicity-split re-approval in step 2. **Orchestrate via workflows where available:** run **dependency-free handoffs as parallel workflows** (each in its own `.loop/`, scopes disjoint), and within a cycle fan out candidate generation and the skeleton code-review by lens. Respect the handoff dependency DAG; never parallelize a handoff with one it depends on. For every handoff, run the **per-handoff autoresearch loop** (full mechanics in `autoresearch-loop.md`):
