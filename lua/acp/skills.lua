...
local function skills_root()
  local override = vim.env.ACP_SKILLS_DIR
  if override and override ~= "" then return override end
  return vim.fn.stdpath("config") .. "/acp-skills"
end
...
local _cache = { mtime = -1, root = nil, skills = {} }
...
local function scan(root)
  ...
  for _, entry in ipairs(entries) do
    ...
    if vim.fn.filereadable(skill_path) == 1 then
      ...
      if parsed then skills[#skills + 1] = parsed end
    end
  end
...
function M.load(_project_root)
  ...
  if _cache.root == root and _cache.mtime == mtime then
    return _cache.skills
  end
  _cache.root, _cache.mtime, _cache.skills = root, mtime, scan(root)
  return _cache.skills
end
...
function M.build_prompt_block(project_root)
  ...
  local lines = {
    "The following custom skills are available in this environment. Each skill is a Markdown file you can Read when the user's request matches its purpose. Do not load them unless relevant.",
    "",
  }
