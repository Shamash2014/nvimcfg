...
function M.normalize_url(url, cwd)
...
function M.list(url, column_defs, cb)
...
function M.perform_action(url, action, cb)
  ...
  if not conn then
    cb("Not connected to " .. parsed.host)
    return
  end

  if action.type == "create" then
    if action.entry_type == "directory" then
      local new_path = path.join(parsed.path, action.url)
      async_ssh.mkdir(conn, new_path, function(success, err)
        cb(err)
      end)
    else
      local new_path = path.join(parsed.path, action.url)
      async_ssh.write_file(conn, new_path, {}, function(success, err)
        cb(err)
      end)
    end
  elseif action.type == "delete" then
    local delete_path = path.join(parsed.path, action.url)
    async_ssh.delete_file(conn, delete_path, function(success, err)
      cb(err)
    end)
  elseif action.type == "move" then
    ...
    if src_parsed and dest_parsed then
      async_ssh.rename(conn, src_parsed.path, dest_parsed.path, function(success, err)
        cb(err)
      end)
    else
...
function M.read_file(url, cb)
  ...
  if not conn then
    cb("Not connected to " .. parsed.host, nil)
    return
  end
...
function M.write_file(url, content, cb)
  ...
  async_ssh.write_file(conn, parsed.path, lines, function(success, err)
    cb(err)
  end)
end
